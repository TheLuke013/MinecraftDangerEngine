...
namespace Minecraft
{
	...
	public:
		...
		//MODULES
		inline std::string GetModuleDescription(unsigned int index) const { return modulesVec->at(index).description; }
		inline ModuleType GetModuleType(unsigned int index) const { return modulesVec->at(index).type; }
		inline std::string GetModuleUuid(unsigned int index) const { return modulesVec->at(index).uuid; }
		inline std::vector<unsigned int> GetModuleVersion(unsigned int index) const { return modulesVec->at(index).version; }
		...
		//MODULES
		inline void SetModuleDescription(unsigned int index, const std::string& newDescription) { modulesVec->at(index).description = newDescription; }
		inline void SetModuleType(unsigned int index, ModuleType newType) { modulesVec->at(index).type = newType; }
		inline void SetModuleUuid(unsigned int index, const std::string& newUuid) { modulesVec->at(index).uuid = newUuid; }
		inline void SetModuleVersion(unsigned int index, std::vector<unsigned int>& newVersion) { modulesVec->at(index).version = newVersion; }
